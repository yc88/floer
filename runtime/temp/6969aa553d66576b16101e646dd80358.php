<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:60:"D:\wwwroot\floer\public/../application/admin\view\login.html";i:1523667161;}*/ ?>
<!DOCTYPE html>
<html lang="en">
...
<div class="signinpanel">
    <div class="row">
        <div class="col-sm-7">
            <div class="signin-info">
                ...
                <div class="m-b"></div>
                <h4>欢迎使用 <strong>admin</strong></h4>
                <ul class="m-b">
        ...
        <div class="col-sm-5" style="margin-left:-100px;">
            <form method="post" action="index.html">
                <p class="m-t-md" id="err_msg">登录到floer后台</p>
                ...
                <div style="margin-bottom:70px">
                    ...
                    <img id="verify" src="<?php echo url('check_verify'); ?>" onclick="javascript:this.src='<?php echo url('check_verify'); ?>?tm='+Math.random();" style="float:right;cursor: pointer"/>
                </div>
    ...
    <div class="signup-footer">
        <div class="pull-left">
            &copy; 2018 04 13  Début du projet
        </div>
    </div>
</div>
...
//            $('#err_msg').hide();
            ...
            var code = $('#code').val();
            $.post("<?php echo url('login/login_go'); ?>",{'user_name':username, 'password':password, 'code':code},function(data){
                lock = false;
                $('#login_btn').val('登录').removeClass('btn-danger').addClass('btn-success');
                if(data.status == 1){
                    layer.msg('验证成功，正在登录。。',function(){
                        window.location.href=data.msg;
                    });
                }else{
                    $('#verify').attr('src', '<?php echo url('check_verify'); ?>?tm='+Math.random());
                    $('#code').val('');
                    layer.msg("<span style='color:red'> "+data.msg+"</span>",{icon: 5});
                }
            });
        });
    });
